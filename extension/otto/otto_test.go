		...
		Context("When extension URL uses file:// protocol", func() {
			It("should read the file and run the extension", func() {
				extension, err := schema.NewExtension(map[string]interface{}{
					"id":   "test_extension",
					"url":  "file://../etc/apps/sample_extension.js",
					"path": ".*",
				})
				Expect(err).ToNot(HaveOccurred())
		...
		Describe("Using chaining builtins", func() {
				...
				Describe("When invoked correctly", func() {
					Context("With transaction", func() {
						...
						It("Correctly lists elements", func() {
							...
							Expect(tx.Commit()).To(Succeed())
							Expect(context["networks"]).To(HaveLen(1))
							resultSlice, ok := context["networks"].([]interface{})
							Expect(ok).To(BeTrue())
							result, ok := resultSlice[0].(map[string]interface{})
							Expect(ok).To(BeTrue())
							testJSONEquality(result, network1)
						})
					})
			...
			Describe("Actually chaining them", func() {
				...
				JustBeforeEach(func() {
					...
					subnetEnv = otto.NewEnvironment(testDB, &middleware.FakeIdentity{})
