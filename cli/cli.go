...
It's especially useful to validate schema files against gohan meta-schema.`,
		Flags: []cli.Flag{
			...
			cli.StringFlag{Name: "document, d", Value: "etc/examples/example.json", Usage: "Document path"},
		},
		Action: func(c *cli.Context) {
...
func getServerCommand() cli.Command {
	return cli.Command{
		...
		Flags: []cli.Flag{
			cli.StringFlag{Name: "config-file", Value: "etc/api_server_config.yaml", Usage: "Server config File"},
		},
		Action: func(c *cli.Context) {
