...
func (gohanClientCLI *GohanClientCLI) getResourceID(s *schema.Schema, identifier string) (string, error) {
	url := fmt.Sprintf("%s%s/%s", gohanClientCLI.opts.gohanEndpointURL, s.Prefix, u.QueryEscape(identifier))
	...
	url = fmt.Sprintf("%s%s?name=%s", gohanClientCLI.opts.gohanEndpointURL, s.Prefix, u.QueryEscape(identifier))
	gohanClientCLI.logRequest("GET", url, gohanClientCLI.provider.TokenID, nil)
