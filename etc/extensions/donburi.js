...
var donburi = {
  plugin: {},
  no_template_keywords: ['eval', 'block', 'define',
                         'resources',
  ...
  run_tasks: function(event_type, context, tasks){
    ...
    _.each(tasks, function(task){
      result = self.run_task(event_type, context, task);
    });
    return result;
  },
  run: function(code){
...
donburi.register_plugin("define", function(event_type, context, value){
   var func_name = value.name;
   var tasks = value.tasks;
   donburi.register_plugin(func_name, function(event_type, context, value){
     var new_context = _.extend(context, value)
     return donburi.run_tasks(event_type, new_context, tasks);
   });
});
...
donburi.register_plugin("resources", function(event_type, context, value){
  if(event_type == "pre_delete" || event_type == "pre_delete_in_transaction"){
    var reversed = value.slice()
    reversed.reverse();
    donburi.run_tasks(event_type, context, reversed);
  }else{
    donburi.run_tasks(event_type, context, value);
  }
});
...
donburi.register_plugin("return", function(event_type, context, value){
  return value;
});
...
donburi.register_plugin("list", function(event_type, context, value){
  ...
  filter.tenant_id = value.tenant_id;
  return gohan_db_list(transaction, value.schema, filter);
});
...
donburi.register_plugin("contrail_fetch", function(event_type, context, value){
    var schema_id = value.schema;
    var id = value.id;
    var token = context.auth_token;
    return contrail_donburi.get_request(token, schema_id, id);
});

donburi.register_plugin("contrail_list", function(event_type, context, value){
    var schema_id = value.schema;
    var token = context.auth_token;
    return contrail_donburi.get_request(token, schema_id);
});
...
var contrail_donburi = {
  ...
  get_request: function(token, schema_id, id){
    var self = this;
    var response = gohan_http(
      "GET",
      self.api_url(schema_id, id),
  ...
  "virtual-network-subnet": function (event_type, context, value){
    ...
    var data = value.properties;
    var network_response = self.get_request(token, "virtual-network",  network_id);
