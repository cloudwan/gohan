...
var _ = Describe("Resource manager", func() {
	...
	Describe("Getting a schema", func() {
		...
		Context("As a member", func() {
			...
			It("Should return trimmed schema when appropriate", func() {
				...
				Expect(ok).To(BeTrue())
				trimmedSchema, err := resources.GetSchema(networkSchema, auth)
				Expect(err).NotTo(HaveOccurred())
				Expect(trimmedSchema).NotTo(BeNil())
			...
			It("Should return no schema when appropriate", func() {
				...
				Expect(ok).To(BeTrue())
				trimmedSchema, err := resources.GetSchema(testSchema, auth)
				Expect(err).NotTo(HaveOccurred())
	...
	Describe("Listing schemas", func() {
		...
		Context("As an admin", func() {
			It("Should return a full list of schemas", func() {
				...
				Expect(err).NotTo(HaveOccurred())
				Expect(number).To(Equal(uint64(11)))
				Expect(result).To(HaveKeyWithValue("schemas", HaveLen(11)))
				schemas, ok := result["schemas"].([]interface{})
				Expect(ok).To(BeTrue())
				Expect(schemas).To(ConsistOf(
					HaveKeyWithValue("id", "schema"),
					HaveKeyWithValue("id", "policy"),
					HaveKeyWithValue("id", "extension"),
					HaveKeyWithValue("id", "event"),
					HaveKeyWithValue("id", "namespace"),
					HaveKeyWithValue("id", "network"),
					HaveKeyWithValue("id", "subnet"),
					HaveKeyWithValue("id", "server"),
					HaveKeyWithValue("id", "test"),
					HaveKeyWithValue("id", "responder"),
					HaveKeyWithValue("id", "admin_only")))
			})
		})
		...
		Context("As a member", func() {
			...
			It("Should return a trimmed list of schemas", func() {
				err := resources.GetMultipleResources(
					context, testDB, currentSchema, map[string][]string{})
				result := context["response"].(map[string]interface{})
				Expect(err).NotTo(HaveOccurred())
				Expect(context["total"].(uint64)).To(Equal(uint64(3)))
				Expect(result).To(HaveKeyWithValue("schemas", HaveLen(3)))
				schemas, ok := result["schemas"].([]interface{})
				Expect(ok).To(BeTrue())
				Expect(schemas).To(ConsistOf(
					HaveKeyWithValue("id", "schema"),
	...
	Describe("Showing a schema", func() {
		BeforeEach(func() {
			schemaID = "schema"
			action = "read"
		})
		...
		Context("As an admin", func() {
			It("Should return the schema", func() {
				err := resources.GetSingleResource(
					context, testDB, currentSchema, "network")
				result := context["response"].(map[string]interface{})
				Expect(err).NotTo(HaveOccurred())
				Expect(result).To(HaveKeyWithValue("schema", HaveKeyWithValue("id", "network")))
			})
		})
		...
		Context("As a member", func() {
			...
			It("Should return a schema when apropriate", func() {
				err := resources.GetSingleResource(
					context, testDB, currentSchema, "network")
				result := context["response"].(map[string]interface{})
				Expect(err).NotTo(HaveOccurred())
			...
			It("Should pretend it knows nothing when apropriate", func() {
				err := resources.GetSingleResource(
					context, testDB, currentSchema, "admin_only")
				...
				Context("Only pre_list_in_transaction", func() {
					...
					It("Should run the extension", func() {
						err := resources.GetMultipleResources(
							context, testDB, currentSchema, map[string][]string{})
						Expect(err).To(HaveOccurred())
						extErr, ok := err.(resources.ExtensionError)
						Expect(ok).To(BeTrue())
		...
		Describe("When there are resources in the database", func() {
			...
			Context("As an admin", func() {
				It("Should return a filled list", func() {
					...
					Expect(result).To(HaveKeyWithValue("tests", HaveLen(2)))
					resources, ok := result["tests"].([]interface{})
					Expect(ok).To(BeTrue())
					Expect(resources).To(ConsistOf(adminResourceData, memberResourceData))
				})
			})
			...
			Context("As a member", func() {
				...
				It("Should return a only owned resources", func() {
					...
					Expect(result).To(HaveKeyWithValue("tests", HaveLen(1)))
					resources, ok := result["tests"].([]interface{})
					Expect(ok).To(BeTrue())
					Expect(resources).To(ConsistOf(adminResourceData))
				})
			})
		})
	})
		...
		Describe("When there are resources in the database", func() {
			...
			Context("As an admin", func() {
				It("Should return owned resource", func() {
					...
					Expect(ok).To(BeTrue())
					testJSONEquality(theResource, adminResourceData)
				})
				...
				It("Should return not owned resource", func() {
					...
					theResource, ok := result[schemaID]
					Expect(ok).To(BeTrue())
					testJSONEquality(theResource, memberResourceData)
				})
			})
	...
	Describe("Executing a sequence of operations", func() {
		...
		It("Should behave as expected", func() {
			...
			Expect(number).To(Equal(uint64(1)))
			Expect(result).To(HaveKeyWithValue("tests", HaveLen(1)))
			resourceArray, ok := result["tests"].([]interface{})
			Expect(ok).To(BeTrue())
			theResource = resourceArray[0]
			testJSONEquality(theResource, adminResourceData)
			delete(listContext, "response")
